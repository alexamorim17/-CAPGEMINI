...
public class exercicio2 {
	...
	public static void main(String[] args) {
		...
		if (senha.length() > 5) { //Verifica se cumpre com os requisitos de caracteres mínimos
			requisito = true;
			contador += 1;
		}
		if (senha.length() >= 1) {
		...
		for (int x = 0; x < senha.length(); x++) {  //Verifica as autenticações
		...
		if (requisito != true && (requisito3 == false || requisito4 == false || requisito5 == false)) {  //Verifica a autenticação devolve o número de caracteres que falta e devolve com uma recomendação.
			System.out.print(senha.length() + " - Você pode tornar a senha mais segura adicionando "
			...
			System.out.println(senha.length()
					+ "- Sua senha cumpre com os requisitos de tamanho, porém não com outros requisitos, tente adicionar caracteres especiais, letra maiúscula e minúscula caso não tenha.");
		}
